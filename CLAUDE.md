...
전체 설계와 커리큘럼은 [docs/study-roadmap.md](docs/study-roadmap.md)가 단일 기준(SSOT)입니다. 작업 전 항상 이 문서의 해당 Phase·토픽을 먼저 확인하세요. 토픽별 집필 진행 상황은 [docs/course-plan.md](docs/course-plan.md)(작성 계획서·체크리스트)로 추적합니다.

## 문서 역할 분담

| 문서 | 역할 |
|------|------|
| [docs/study-roadmap.md](docs/study-roadmap.md) | **SSOT** — 커리큘럼 전체 설계, Phase·토픽 슬러그·번호, 레퍼런스 스택, 데이터셋·논문 URL |
| [docs/course-plan.md](docs/course-plan.md) | **작성 계획서** — 토픽별 산출물 5종 체크리스트, 표준 작성 절차, 진행 추적 |
| 본 문서(CLAUDE.md) | **작업 가이드** — 핵심 원칙·컨벤션·작성 표준 요약(상세는 위 두 문서) |
...
교육자료는 사람이 쓴 것처럼 읽혀야 합니다. 한 토픽은 **`/kb-course-author` 스킬**로 집필하는 것을 기본으로 합니다. 스킬이 다음을 오케스트레이션합니다.

1. **로드** — roadmap의 해당 Phase·토픽 + 공유 레퍼런스 키트를 읽고, 빠르게 바뀌는 버전·API(Neo4j 5.26 LTS·LightRAG·VoyageAI·Ragas 등)만 선별 검증.
2. **초안 집필** — `kb-agent` 에이전트가 `lesson.md` 초안 + `practice/`(실행 가능한 코드) + `labs/`(단계별 핸즈온 + 예상 출력)를 표준 템플릿대로 작성하고 정적 검증. **이 단계에서 윤문은 하지 않습니다.**
3. **윤문** — 곧바로 `/humanize-korean`으로 `lesson.md` **본문만** 윤문해 AI가 쓴 듯한 느낌(번역투·기계적 병렬·접속사 남발·이모지/불릿 과다 등)을 완화.
4. **보존 검증·보고** — 윤문 전후 diff로 보존 대상 불변을 확인하고 완료 기준·경로를 보고. 이후 [docs/course-plan.md](docs/course-plan.md)의 체크박스를 갱신.

> 후속 요청도 같은 스킬입니다 — "이 토픽 다시"(1단계부터), "practice만 보강"(kb-agent 재호출), "윤문 다시"(3–4단계만).

> ⚠️ **윤문 철칙**: `/humanize-korean`은 **문체·리듬·표현만** 바꾸고 **내용·코드·수치·출처·용어는 한 글자도 건드리지 않습니다.** 코드 블록·명령어·완료 기준·URL·영문 약어·토픽 슬러그는 윤문 대상에서 제외하고, 윤문 후 실습 코드가 그대로인지 반드시 확인합니다.
...
- 토픽 집필은 **`/kb-course-author` 스킬** 호출을 기본으로 합니다(상세 절차는 [docs/course-plan.md](docs/course-plan.md)).
- 새 토픽을 만들 때는 study-roadmap.md의 토픽 슬러그·번호를 그대로 따릅니다(임의 변경 금지).
- 도구·버전·데이터셋·논문 URL은 study-roadmap.md의 "자료"·"참고 문헌" 섹션과 일치시킵니다.
- 토픽을 완료하면 [docs/course-plan.md](docs/course-plan.md)의 해당 체크박스를 `[x]`로 갱신합니다.
