...
스키마를 여기서 다시 정의하지 않는다. 추출 타깃은 2/01 에서 이미 못 박았다:
  Entity / NodeType / Provenance / GraphSchemaSample.
이 토픽은 그 모델을 '추출 결과로 채우는' 단계다. 같은 스키마를 두 번 쓰면
나중에 두 곳이 어긋난다 — 단일 진실 원천(2/01)을 import 로 재사용한다.
...
# 이 파일(.../02-.../practice/schema_adapter.py) 기준으로 2/01 practice 경로를 계산한다.
...
__all__ = ["Entity", "GraphSchemaSample", "NodeType", "Provenance"]
